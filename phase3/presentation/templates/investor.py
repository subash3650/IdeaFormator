...
@register_template(name="investor")
class InvestorTemplate(BusinessTemplate):
    @property
    def name(self) -> str:
        return "investor"
    ...
    def sections(self) -> list[SectionDefinition]:
        return [
            ...
            SectionDefinition(section_type=SectionType.trend_analysis, title="Trend Analysis", order=3),
            SectionDefinition(section_type=SectionType.confidence, title="Confidence", order=4),
    ...
    def chart_types(self) -> list[ChartType]:
        return [ChartType.bar, ChartType.pie, ChartType.treemap]

    def title(self, data: dict[str, Any]) -> str:
        return data.get("title", "Investor Report")

    def subtitle(self, data: dict[str, Any]) -> str:
        return data.get("subtitle", "Market opportunities and growth analysis for investors")
