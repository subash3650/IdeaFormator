...
@register_template(name="executive")
class ExecutiveTemplate(BusinessTemplate):
    @property
    def name(self) -> str:
        return "executive"
    ...
    def sections(self) -> list[SectionDefinition]:
        return [
            ...
            SectionDefinition(section_type=SectionType.confidence, title="Confidence", order=5),
    ...
    def chart_types(self) -> list[ChartType]:
        return [ChartType.bar, ChartType.pie, ChartType.line]

    def title(self, data: dict[str, Any]) -> str:
        return data.get("title", "Executive Summary Report")

    def subtitle(self, data: dict[str, Any]) -> str:
        return data.get("subtitle", "High-level overview of key findings")
